...
class IndepQueueManager(TaskQueueManagerBase):
    """Manage a set of independent limited task queues.

    A task can only belong to one queue. Queues release tasks if the number of
    active tasks in its membership list is below its queue limit, until the
    ...
    def _make_indep(self, in_queues: dict) -> dict:
        ...
        Latest queue assignment takes precedence. The "default" queue contains
        tasks not assigned to another queue.
