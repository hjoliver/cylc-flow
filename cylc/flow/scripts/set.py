...
Examples:
  ...
  # set all prequisites to cycle point 3 (use to skip cycles):
  $ cylc set --pre=cycle my_workflow//3
...
def get_option_parser() -> COP:
    ...
    parser.add_option(
        ...
        help=(
            ...
            " Use 'cycle' to satisfy whole-cycle prerequisites,"
            " e.g. to skip cycle points."
        ),
...
def validate_prereq(prereq: str) -> bool:
    ...
    if (
        tokens["cycle"] == prereq
        and prereq not in ["all", "cycle"]
    ):
        # Error: --pre=<word> other than "all" or "cycle"
        return False
    return True
...
def get_prerequisite_opts(prereq_options: List[str]):
    ...
    if len(prereqs) > 1:
        ...
        if "cycle" in prereqs:
            raise InputError("--pre=cycle must be used alone")
